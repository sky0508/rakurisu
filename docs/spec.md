...
## 7. バックエンド連携アーキテクチャ 【未決・要 Sora 判断】

ラクリスが lead-harvest パイプラインをどう実行・観測するかは、フロントの作り方を左右する分岐。

| 案 | 実行 | 状態の持ち方 | 長所 | 短所 |
|---|---|---|---|---|
| **A. ローカル常駐（推奨）** | Next.js を Sora の Mac で起動。API から Python スクリプトを spawn。JSONL/ログを読む | ファイル（JSONL）+ 軽い SQLite | workload-board 前例あり・実装最小・クロールは元々ローカル | Sora の Mac 稼働時だけ・社内閲覧は別途 |
| **B. DB 集約 + ローカル実行** | 実行はローカル、状態は Python が DB（Neon 等）に書く。webapp は DB を読む | Neon 等 | 社内から閲覧可・ハンドオフ向き | env/DB セットアップ増 |
| **C. フルクラウド** | クロールもクラウドで | クラウド DB | — | 長時間クロール・robots/コスト的に非現実的。不採用 |

**推奨: A で P1 を作り、社内閲覧ニーズが立ったら B へ拡張**（YAGNI・「今は最小、将来の拡張口だけ残す」）。
→ この判断が確定するまで P1 の plan.md は書かない。
...
1. **§7 の実行アーキテクチャ**（A / B）← 最重要。これが決まれば P1 の plan へ進める
2. ジョブの案件ディレクトリ（`03_areas/alphadrive/リスト作成/<案件>/`）とラクリスの紐付け方
3. `queued` / `failed` 状態を P1 に含めるか
4. 社内閲覧を P1 で必要とするか（→ 案 B を前倒すか）
